...
// ── DYNAMIC PRODUCT FROM HOME PAGE ──
// Home page kirim query: ?title=...&price=...&img=...
(function applyProductFromQuery() {
  ...
  const artistEl = document.getElementById('productArtist');
  const priceEl = document.getElementById('productPrice');
  const imgEl = document.getElementById('productImage');

  if (titleEl && title) titleEl.textContent = title;

  // artist saat ini belum dikirim terpisah; kalau tidak ada, pakai default
  if (artistEl) {
    // heuristik: ambil bagian setelah '-' jika ada
    if (title && title.includes('-')) {
      artistEl.textContent = title.split('-').slice(1).join('-').trim();
    }
  }

  if (priceEl && price) {
    const digits = String(price).replace(/\D/g, '');
    if (digits) priceEl.textContent = 'Rp ' + digits;
  }
